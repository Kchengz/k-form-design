...
// CONCATENATED MODULE: ./packages/utils/antdStyle.js
...
// CONCATENATED MODULE: ./packages/utils/getPluginManager.js
// 发布npm 需要切换到这个
// export { pluginManager } from "k-form-design";
// export { revoke } from "k-form-design";
// 本地测试


// CONCATENATED MODULE: ./packages/KFormDesign/config/formItemsConfig.js
/*
 * author kcz
 * date 2019-11-20
 * description 表单控件项
...
function setFormDesignConfig(config) {
  ...
  try {
    ...
    customComponents.list = config.list || []; // 存储自定义组件

    var customComponentList = config.list || [];
    customComponentList.forEach(function (item) {
      pluginManager.addComponent(item.type, item.component);
    });
    pluginManager; // uploadFile 配置 start
    // 配置uploadFile默认上传地址

    var uploadFile = basicsList.filter(function (item) {
      return item.type === "uploadFile";
    })[0];
    ...
    var uploadImg = basicsList.filter(function (item) {
      return item.type === "uploadImg";
    })[0];
...
/* harmony default export */ var entry_lib = __webpack_exports__["default"] = (/* Cannot get final name for export "default" in "./packages/mini.js" (known exports: setFormDesignConfig setFormBuildConfig, known reexports: pluginManager revoke) */ undefined);
